...
from .task_head import LogisticRegressionHead, MultiChoiceHead
...
class AlbefForMultiTask(nn.Module):
    def __init__(
        ...
        self.cross_text = config["cross_text"]
        ...
        self.text_encoder = BertModel.from_pretrained(
            config["text_encoder"], config=bert_config, add_pooling_layer=False
        )

        # multi-task cls head
        self.task_head_dict = {}
        if self.use_info_type_cls:
            self.info_cls_head = MultiChoiceHead(bert_config, config["info_label_nums"])
        ...
        if self.distill:
            ...
            if self.use_info_type_cls:
                ...
                self.model_pairs.append([self.info_cls_head, self.info_cls_head_m])
                self.task_head_dict_m.update({"info_cls_type": self.info_cls_head_m})
            if self.use_priority_regression:
                self.priority_regression_head_m = LogisticRegressionHead(bert_config)
                self.model_pairs.append(
                    [self.priority_regression_head, self.priority_regression_head_m]
                )
    ...
    def forward(
        ...
        **kwargs,
        ...
        image_attention_mask = image_attention_mask.to(image.device)

        assert image_embeds.shape[:-1] == image_attention_mask.shape, (
            "image_attention_mask.shape"
            f" is expected to be {image_embeds.shape[:-1]}"
            f" but got {image_attention_mask.shape}"
        )
        ...
        if train:
            task_loss = {
                task: task_output.loss for task, task_output in mtl_outputs_dict.items()
            }
            if self.distill:
                # 如果动态蒸馏
                with torch.no_grad():
                ...
                loss_dict = {}
                for task in mtl_outputs_dict.keys():
                    loss = (1 - alpha) * task_loss[task] + alpha * distill_loss[task]
        ...
        return {
            task: task_output.logits for task, task_output in mtl_outputs_dict.items()
        }
